...
### Класс `DrawingToolStrategy` (абстрактный базовый класс)
...
Абстрактный метод, определяющий интерфейс для всех инструментов рисования. Принимает холст, начальные и конечные координаты, дополнительные параметры (например, цвет) и возвращает ID созданного объекта на холсте.
...
#### `draw(self, canvas: tk.Canvas, start_x: int, start_y: int, end_x: int, end_y: int, **kwargs: Any) -> int`
Рисует линию на холсте от точки (start_x, start_y) до точки (end_x, end_y). Извлекает цвет из kwargs (по умолчанию 'blue'), создает линию с толщиной 2 пикселя и возвращает ID созданного объекта.
